...
生成物のうち `apm.lock.yaml` と `.github/instructions/{pr-review,language}.instructions.md`（base 由来・クラウド Copilot 経路確保のための例外）は追跡対象としてコミットする。それ以外の生成物（`.github/instructions/` の他ファイル・`AGENTS.md` / `CLAUDE.md` / `.claude/rules/` など）は `.gitignore` 対象のためコミットに含まれない。
